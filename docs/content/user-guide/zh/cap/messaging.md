...
7.1.0 版本引入了基于数据库的分布式锁，用于降低多个实例下重试操作并发获取数据库数据的问题。你需要显式配置 `UseStorageLock` 为 true 才会启用该能力。该锁保护的是重试消息的获取过程，并不是集群范围内针对单条消息的重试限流器；在多实例部署中，持续失败的消息仍可能在后续轮询周期被不同实例重试。
