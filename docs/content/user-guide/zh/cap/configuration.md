...
!!! WARNING "多实例并发重试"
    我们在7.1.0版本中引入了基于数据库的分布式锁，用于降低多个实例下重试进程并发获取数据库数据导致的重复获取问题，你需要显式配置 `UseStorageLock` 为 true。
    `UseStorageLock` 用于协调每个重试处理器获取数据库消息的过程，但它不会对单条持续失败的消息提供集群范围内的节流保证。在多实例部署中，持续失败的消息可能在后续轮询周期被不同实例重新获取，因此实际观察到的重试频率可能会受到副本数量影响。

#### UseStorageLock

> 默认值: false

如果设置为true，我们将使用基于数据库的分布式锁以应对重试进程在多个实例下对数据库数据的并发获取问题。这将会在数据库生成 cap.lock 表。该锁只保护重试消息的获取过程；它不保证每条失败消息在整个集群中最多每 `FailedRetryInterval` 重试一次。
