...
| AutoProvision                        | Automatically create topics, subscriptions, and rules. Disable when the Service Bus administration API is unavailable (for example, with the emulator).                 | bool                                                                   | true                             |
...
| TokenCredential                      | Azure identity credential used for authentication. Set `Namespace` when using this option.                                                                              | TokenCredential                                                        | null                             |
| MaxConcurrentCalls                   | Maximum concurrent message handler calls.                                                                                                                             | int                                                                    | 1                                |
...
| CustomHeadersBuilder                 | Adds custom and/or mandatory headers for incoming messages from heterogeneous systems.                                                                               | `Func<ServiceBusReceivedMessage, IServiceProvider, List<KeyValuePair<string, string>>>?` | null                 |
