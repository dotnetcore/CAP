...
namespace Sample.RabbitMQ.Spanner.Controllers
{
    ...
    public class ValuesController : Controller
    {
        ...
        {
            using (var connection = new SpannerConnection(AppDbContext.ConnectionString))
            {
        ...
        public IActionResult EntityFrameworkWithTransaction([FromServices] AppDbContext dbContext)
        {
            ...
            {
                dbContext.Persons.Add(new Person() { Id = DateTime.UtcNow.Second , Name = "ef.transaction" });

                for (int i = 0; i < 1; i++)
                {
                    _capBus.Publish("sample.rabbitmq.spanner", DateTime.Now);
                }
