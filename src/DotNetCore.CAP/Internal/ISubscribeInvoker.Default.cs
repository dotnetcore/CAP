...
{
    public partial class SubscribeInvoker : ISubscribeInvoker
    {
        private readonly ILogger _logger;
        private readonly IServiceProvider _serviceProvider;
        private readonly ConcurrentDictionary<string, ObjectMethodExecutor> _executors;

        public SubscribeInvoker(ILoggerFactory loggerFactory,
        ...
        public async Task<ConsumerExecutedResult> InvokeAsync(ConsumerContext context, CancellationToken cancellationToken = default)
        {
            ...
            for (var i = 0; i < parameterDescriptors.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if (message.Value is JToken jToken)  // use ISerializer when reading from storage, skip other objects if not Json
                        {
    ...
    public partial class SubscribeInvoker : ISubscribeInvoker
    {
        public async Task<ConsumerExecutedResult> InvokeAsync<T>(ConsumerContext<T> context, CancellationToken cancellationToken = default)
            where T : new()
        {
            cancellationToken.ThrowIfCancellationRequested();
        ...
        protected virtual object GetInstance<T>(IServiceProvider provider, ConsumerContext<T> context)
        {
            var srvType = context.ConsumerDescriptor.ServiceTypeInfo?.AsType();
            var implType = context.ConsumerDescriptor.ImplTypeInfo.AsType();
